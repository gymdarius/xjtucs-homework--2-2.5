...
void Scene::simulation_update()
{
    ...
    while (remained_duration > (duration)time_step)
    {
        for (auto each_object : all_objects)
            each_object->update(all_objects);
        remained_duration = remained_duration - (duration)time_step;
        time_count++;
    }
    last_update = last_update + std::chrono::duration_cast<std::chrono::steady_clock::duration>((duration)time_count * time_step);
}   
