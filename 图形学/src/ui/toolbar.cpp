...
void Toolbar::layout_mode(Scene& scene)
{
    if (ImGui::BeginTabItem("Layout")) {
        ...
        if (selected_object != nullptr) {
            ...
            // 旋转角度的一半
            float half_theta_z = 0.5f * radians(z_angle);
            float half_theta_x = 0.5f * radians(x_angle);
            float half_theta_y = 0.5f * radians(y_angle);

            // 分别计算三个轴的 sin 和 cos 值
            float sin_half_theta_z = sin(half_theta_z);
            float cos_half_theta_z = cos(half_theta_z);
            float sin_half_theta_x = sin(half_theta_x);
            float cos_half_theta_x = cos(half_theta_x);
            float sin_half_theta_y = sin(half_theta_y);
            float cos_half_theta_y = cos(half_theta_y);

            // 计算每个轴的四元数
            Quaternionf z_Quaternion(cos_half_theta_z, 0.0f, 0.0f, sin_half_theta_z);
            Quaternionf x_Quaternion(cos_half_theta_x, sin_half_theta_x, 0.0f, 0.0f);
            Quaternionf y_Quaternion(cos_half_theta_y, 0.0f, sin_half_theta_y, 0.0f);

            // 应用旋转，注意顺序(四元数乘法不满足交换律)
            quaternion = x_Quaternion * y_Quaternion * z_Quaternion * quaternion;

            //将四元数赋值给选定对象的旋转属性
            selected_object->rotation = quaternion;
        }
        ImGui::EndTabItem();
    }
}
